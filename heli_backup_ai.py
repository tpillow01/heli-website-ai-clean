...
def chat():
    try:
        ...
        app.logger.info(f"/api/chat start mode={mode} qlen={len(user_q)}")
        ...
        if mode == "inquiry":
            try:
                ...
                app.logger.exception("Import failed for inquiry helpers")
                return jsonify({"error": f"inquiry import failed: {imp_err}"}), 500

            try:
                probe = find_customer_id_by_name(user_q) or user_q
            except Exception as id_err:
                app.logger.exception("find_customer_id_by_name failed")
                return jsonify({"error": f"customer-id lookup failed: {id_err}"}), 500

            try:
                brief = build_inquiry_brief(probe)
            except Exception as brief_err:
                app.logger.exception("build_inquiry_brief crashed")
                return jsonify({"error": f"failed to build inquiry brief: {brief_err}"}), 500

            if not brief:
                return jsonify({"response": "I couldn’t locate that customer in the report/billing data. Please use the exact company name as it appears in your system."}), 200

            # --- OpenAI call ---
            system_prompt = {
                "role": "system",
                "content": (
                    "You are a sales strategist for a forklift dealership. Use the INQUIRY context verbatim; "
                    "do not invent numbers. Output a concise, organized brief:\n\n"
                    "1) Segmentation — Account Size (A/B/C/D) and Relationship (1/2/3/P) with meanings.\n"
                    "2) Current Pattern — When they spend (top months), what they buy (top offerings), and frequency.\n"
                    "3) Visit Plan — What to lead with (Service/Parts/Rental/New/Used) and why based on billing.\n"
                    "4) Move from X→Y — Concrete steps to reach the next better tier (e.g., B3→B2), offerings to add.\n"
                    "5) Next Actions — 3 short, specific tasks for today.\n"
                    "Be practical. Prefer checklists. If any data is missing, state it plainly."
                )
            }
            messages = [
                system_prompt,
            ...
            try:
                ...
                return jsonify({"error": f"OpenAI error: {llm_err}"}), 500
        ...
        system_prompt = {
            ...
            "content": (
                ...
                "Use these sections: Customer Profile:, Model:, Power:, Capacity:, Tire Type:, "
                "Attachments:, Comparison:, Sales Pitch Techniques:, Common Objections:.\n"
                "Only cite forklift Model codes exactly as they appear (e.g. CPD25, CQD16)."
            )
        }
        ...
        try:
            ...
            app.logger.exception("OpenAI call failed (recommendation)")
            return jsonify({"error": f"OpenAI error: {llm_err}"}), 500

        return jsonify({"response": ai_reply}), 200

    except Exception as e:
        app.logger.exception("/api/chat crashed")
