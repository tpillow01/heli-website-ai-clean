...
POWER_KEYS = [
    "Power", "power", "Fuel", "fuel", "Drive", "Drive Type",
...
def _rows_from_df() -> List[Dict[str, Any]]:
    ...
    for _, r in df.iterrows():
        ...
        if not name:
            continue
        out.append(
            {"name": name, "benefit": ben, "type": typ, "subcategory": sub}
        )
    return out
...
def _env_flags(q: str) -> Dict[str, bool]:
    ...
    return {
        ...
        "indoor": any(
            k in ql
            for k in ("indoor", "warehouse", "inside", "epoxy", "polished", "concrete")
        ),
        "outdoor": any(
            k in ql
            for k in ("outdoor", "yard", "rain", "snow", "dust", "gravel", "dirt")
        ),
        "dark": any(
            k in ql for k in ("dark", "dim", "night", "poor lighting", "low light")
        ),
        "mentions_clamp": bool(
            re.search(r"\bclamp|paper\s*roll|bale|drum|carton|block\b", ql)
        ),
        "mentions_align": bool(
            re.search(r"\balign|tight\s*aisle|narrow|staging\b", ql)
        ),
        "mentions_widths": bool(
            re.search(r"\bvar(y|ied)\s*width|mixed\s*pallet|different\s*width\b", ql)
        ),
...
def _kw_score(q: str, name: str, benefit: str) -> float:
    ...
    if any(k in ql for k in ("cold", "freezer", "subzero", "winter")):
        ...
        if "air conditioner" in text or "a/c" in text:
            score -= 2.0
        if any(
            k in ql for k in ("dark", "dim", "poor lighting", "night")
        ) and any(k in text for k in ("light", "led", "beacon", "work light")):
            score += 1.2
...
def _rank_bucket(
    q: str, bucket: Dict[str, str], limit: int = 6
) -> List[Dict[str, str]]:
    if not bucket:
        return []
...
def _prioritize_lighting(
    items: List[Dict[str, str]], q_lower: str
) -> List[Dict[str, str]]:
    if not any(
        k in q_lower for k in ("dark", "dim", "night", "poor lighting", "low light")
    ):
        return items
...
def _drop_ac_when_cold(
    ...
    return [
        x
        for x in items
        if "air conditioner" not in (x.get("name", "") + " " + x.get("benefit", "")).lower()
    ]


def recommend_options_from_sheet(
    user_q: str, limit: int = 6
    ...
    telemetry = {
        n: b for n, b in options.items() if _TELEM_PAT.search((n + " " + b).lower())
    }
    ql = (user_q or "").lower()
    ...
    if wants["any"]:
        if wants["tires"]:
            ...
            if env["asks_non_mark"]:
                filtered = [
                    t
                    for t in ranked
                    if "non-mark" in (t["name"] + " " + t.get("benefit", "")).lower()
                ]
        ...
        if wants["attachments"]:
            ...
            if env["indoor"] and not env["mentions_clamp"]:
                atts = [
                    a for a in atts if not re.search(r"\bclamp\b", a["name"].lower())
                ]
            if env["indoor"]:
                atts.sort(
                    key=lambda a: int(
                        ("sideshifter" in a["name"].lower())
                        or ("fork positioner" in a["name"].lower())
                    ),
                    reverse=True,
                )
...
def parse_catalog_intent(user_q: str) -> Dict[str, Any]:
    ...
    list_all = bool(
        re.search(
            r"\b(list|show|give|display)\b.*\b(all|full|everything)\b",
            t,
        )
    )
    ...
    if intent["list_all"] and which in ("tires", "attachments", "options", "telemetry"):
        if which == "telemetry":
            ...
            telem = {
                ...
                if _TELEM_PAT.search((n + " " + b).lower())
            }
...
def allowed_models_block(allowed: Iterable) -> str:
    ...
    for m in allowed[:15]:
        ...
        lines.append(
            f"- {code} ({cap_txt}, {power.upper() if isinstance(power, str) else power})"
        )
    return "\n".join(lines)
...
def _wants_all_terrain(user_q: str) -> bool:
    return bool(
        re.search(
            r"\b(all[-\s]?terrain|rough\s*terrain|yard|outdoor)\b",
            user_q or "",
            re.I,
        )
    )
...
def _wants_indoor_epoxy(user_q: str) -> bool:
    return bool(
        re.search(r"\b(indoor|warehouse|epoxy|polished)\b", user_q or "", re.I)
    )
...
def _model_score(user_q: str, row: Dict[str, Any]) -> float:
    ...
    if _wants_indoor_epoxy(user_q):
        if "cushion" in tire:
            score += 1.2
        if "non" in tire and "mark" in tire:
            score += 1.0
    ...
    if re.search(r"\baisle\b", ql) and aisle:
        score += 0.6
    if re.search(r"\bheight|lift\b", ql) and h_in:
        score += 0.6
...
def generate_forklift_context(
    ...
    if acct and isinstance(acct, dict):
        ...
        if name:
            acct_line = f"\nAccount: {name}"
    return (
        f"User question: {user_q.strip()}{acct_line}\n"
