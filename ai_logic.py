...
def generate_forklift_context(user_input: str, customer_name: str) -> str:
    ...
    # Build full Models Data block
    model_lines = ["<span class=\"section-label\">Available Models:</span>"]
    for m in models_data:
        model_lines.append(
            f"- {m.get('Model','N/A')} | Type: {m.get('Type','N/A')} | "
            f"Power: {m.get('Power','N/A')} | Capacity (lbs): {m.get('Capacity_lbs','N/A')} | "
            f"Height (in): {m.get('Height_in','N/A')} | Lift Height (in): {m.get('LiftHeight_in','N/A')}"
        )
    model_block = "\n".join(model_lines)
    ...
    parts.append(user_input)   # user's actual question
